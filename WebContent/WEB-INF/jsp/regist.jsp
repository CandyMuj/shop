...
<script type="text/javascript">
	...
	function checkUserName(){
		...
		$("#span1").load("${pageContext.request.contextPath}/user_checkUserName.action?"+new Date().getTime(),{'username':username}); //json格式的数据
		/* 
			load方法 直接将服务器响应的回显数据加载到某个元素 不需要你再写回调函数 得到text 他直接给你加载 自动内部调用了
		*/
	}
	
	function change() {
		document.getElementById("checkImg").src="${pageContext.request.contextPath}/checkImg.action?"+new Date().getTime();//有缓存所以需要加一个时间戳，以后只要执行什么他没有变化 那么一般都是执行的缓存
	}
	...
	function sure(){
		...
		if (repassword != password) {
			document.getElementById("spanr").innerHTML="两次密码不一致";
		}else{
			document.getElementById("spanr").innerHTML="";
		}
	}
			...
			<div class="wrap">
				<div class="main clearfix">
					...
					<form id="registerForm"
						...
						method="post"
						onsubmit="return checkForm();"><!-- 在表单中如果提交事件 返回的是FALSE 将不会提交数据到服务器 会阻止数据的提交 -->
						<table>
							<tbody>
								<tr>
									...
									<td><input type="text" id="username" name="username"
										...
										id="span1" style="margin-left: 5px;"><s:fielderror fieldName="username" /><!-- 获取错误信息是提交后访问action才获取  ajax的校验是不用提交失去焦点就会执行 -->
									</span></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td><input type="password" id="repassword"
										...
										onblur="sure()"/>
										<font color="red" style="margin-left: 5px;"><span id="spanr"></span></font>
										</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<input type="text"
											...
											<!-- 只要加载这个页面就会请求这个路径然后他给你响应一个激活码 -->
											<img id="checkImg" class="checkImg"	src="${pageContext.request.contextPath}/checkImg.action" 
											title="点击更换验证码" onclick="change()"/>
											<span><font color="red" style="margin-left: 5px;"><s:actionerror /> </font></span>
									</span></td>
								</tr>
								<tr>
									<th>&nbsp;</th>
